	...
	<section class="content">
		<div class="row">
			<div class="col-md-12">
				...
				<a href="{{ asset('/all-documents/create') }}" type="button" class="btn btn-success btn-flat margin">New Document</a>
				<div class="box box-success">
					...
					<div class="box-body table-responsive no-padding">
						<table class="table table-hover dataTable">
							...
							<tr>
								<td>{{ $document->id }}</td>
								<td><a href="{{ $document->user ? asset('/users/'.$document->user->id) : '' }}"></a>{{ $document->user ? $document->user->first_name.' '.$document->user->last_name : '' }}</td>
								<td>{{ $document->document_type ? $document->document_type->document_type : '' }}</td>

								<td>
									<div class="btn-group">
										<a href="{{ asset('all-documents/'.$document->id) }}" title="Download" class="btn btn-default btn-flat btn-sm"><i class="fa fa-download"></i></a>
										@if(Auth::user()->role->role_permission('edit_documents') || Auth::user()->role->role_permission('delete_documents'))
										@if(Auth::user()->role->role_permission('edit_documents'))
										<a href="{{ asset('all-documents/'.$document->id.'/edit') }}" title="Edit" class="btn btn-default btn-flat btn-sm"><i class="fa fa-pencil"></i></a>
										...
										<div id="deleteModal{{ $document->id }}" class="modal fade" role="dialog">
											...
											{!! Form::open(['url' => ['all-documents/'.$document->id], 'method' => 'delete']) !!}
